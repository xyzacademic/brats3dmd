...
class Brats19Segmenter(Model):
        ...
        for batch_idx, (data, target) in enumerate(train_loader):
            ...
            if config['flip']:
                if np.random.randn() > 0.5:
                    axes = np.random.choice([2, 3, 4], np.random.choice([1, 2, 3], 1), False)
                    for axis in axes:
                        data = data.flip(axis)
                        target = target.flip(axis)

            self.optimizer.zero_grad()
            outputs, wt, tc, et, ed, ncr = self.net(data)
            loss = self.criterion(outputs, wt, tc, et, ed, ncr, target[:, 0], target[:, 1], target[:, 2], target[:, 3], target[:, 4], target[:, 5])
            if self.use_apex:
            ...
            if em_list is not None:
                ...
                value = evaluation(predict, target[:, 0], 'dice_brats19')
    ...
    def test(self, epoch=0, test_loader=None, em_list=None, verbose=True):
        ...
        with torch.no_grad():
            for batch_idx, (data, target) in enumerate(test_loader):
                ...
                outputs, wt, tc, et, ed, ncr = self.net(data)
                loss = self.criterion(outputs, wt, tc, et, ed, ncr, target[:, 0], target[:, 1], target[:, 2],
    ...
    def inference2(self, test_loader=None, prob=False, target_path=None, replace_origin='anat', replace_target='pred'):
        config = self.config
        assert self.net is not None
        root = test_loader.dataset.root
        self.net.eval()
        ...
        with torch.no_grad():
            for batch_idx, (data, filenames) in enumerate(test_loader):
                ...
                for i in range(pre.shape[0]):
                    patient = nib.load(os.path.join(root, filenames[i]))
                    new_pre = np.zeros(shape=(data.size(0), 91, 109, 91), dtype=np.uint8)
                    new_pre[:, :, :, :] = pre[:, 5:, 19:, 5:]
                    new_data = nib.Nifti1Image(new_pre[i], patient.affine, patient.header)
                    nib.save(new_data, os.path.join(target_path, filenames[i].replace(replace_origin, replace_target)))
                    print('Save {file} successfully.'.format(file=os.path.join(target_path,
