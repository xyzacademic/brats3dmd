...
class SCD(object):
    ...
    def __init__(self, nrows, nfeatures, w_inc1=0.1, w_inc2=0.1, tol=0.001, local_iter=10,
    ...
    def train(self, data, labels, val_data=None, val_labels=None,
        ...
        for r in range(self.round):
            if warm_start and self.w_index != []:
                ...
                w1 = np.random.uniform(-1, 1, size=(num_cols, self.hidden_nodes)).astype(np.float32)
                w2 = np.random.uniform(-1, 1, size=(self.hidden_nodes, )).astype(np.float32)
            results.append(
                pool.apply_async(self.single_run, args=(train_data, train_labels, plus, minus, val_data, val_labels, w1, w2, column_indices, r % self.num_gpus)))
    ...
    def single_run(self, data, labels, plus, minus, val_data, val_labels, w1, w2,
        ...
        return w1.cpu(), b1.cpu(), w2.cpu(), b2.cpu(), best_acc

    def single_iteration(self, w1, w2, data, labels, plus, minus):
        """
        ...
        best_acc = 0
        initial_b1, initial_b2, best_acc = self.get_init_bias(data, labels, w1, w2, plus, minus)
        best_w2, best_b2 = self.get_best_w2_and_b2(data, labels, w1, w2, initial_b1, plus, minus, best_acc)
        best_w1, best_b1 = self.get_best_w1_and_b1(data, labels, w1, best_w2, initial_b1, best_b2, plus, minus, best_acc)
    ...
    def get_best_w1_and_b1(self, data, labels, w1, w2, b1, b2, plus, minus, best_objective):
        """
        ...
        idx = np.random.choice(np.arange(self.hidden_nodes), 1)
        b1[idx] = 0
        temp_w2 = w2.clone()
        temp_w2[idx] = 0
        ...
        p2_sign = ((self.obtain_projection((p1[raw_index_sorted].sign() + 1) / 2, temp_w2).view((1, -1)) + b2 + self.yp.cuda(self.device) * w2[idx]).sign() + 1) // 2
        best_acc, b1_i, best_index = self.get_best_b1(labels[raw_index_sorted], pi[raw_index_sorted], None, None, plus, minus, False, p2_sign)
        b1[idx] = b1_i
        del p2_sign, pi
        localit = 0
        ...
        while localit < self.local_iter and \
            ...
            end_index = min(temp_labels.shape[1], best_temp_index + self.interval)
            p2_sign = ((p2_sign[raw_index_sorted].unsqueeze(dim=1) + self.yp[start_index: end_index].unsqueeze(dim=0).cuda(self.device) * w2[idx]).sign() + 1) // 2
            temp_acc, temp_b, temp_row, temp_index = self.get_best_b1(temp_labels, projection, best_temp_index, self.interval, plus, minus, True, p2_sign)
    ...
    def get_init_bias(self, data, labels, w1, w2, plus, minus):
        ...
        projection1 = self.obtain_projection(data, w1)
        init_b1 = projection1.mean(dim=1)
        projection2 = self.obtain_projection(((projection1 + init_b1.view((1, -1))).sign() + 1) / 2, w2)
        raw_index_sorted = self.argsort(projection2)
        projection2 = projection2[raw_index_sorted]
        best_acc, b2, best_b2_index = self.get_best_b2(labels[raw_index_sorted], projection2, None, None, plus, minus)
        ...
        for i in np.random.permutation(self.hidden_nodes):
            ...
            temp_w2 = w2.clone()
            temp_w2[i] = 0
            p1 = projection1 + init_b1
            pi = projection1[i]
            raw_index_sorted = self.argsort(pi)
            p1 = p1[raw_index_sorted]
            p2_sign = ((self.obtain_projection((p1.sign()+1)/2, temp_w2).view((1, -1)) + b2 + self.yp.cuda(self.device) * w2[i]).sign() + 1) // 2
            best_acc, b1_i, best_b1_i_index = self.get_best_b1(labels[raw_index_sorted], pi[raw_index_sorted], None, None, plus, minus, False, p2_sign)
            init_b1[i] = b1_i
    ...
    def get_best_w2_and_b2(self, data, labels, w1, w2, initial_b1, plus, minus, best_objective):
        """
        ...
        :return:
        """
        p1_sign = ((self.obtain_projection(data, w1) + initial_b1.view((1, -1))).sign() + 1) / 2
    ...
    def get_best_b2(self, labels, projection, index, interval, plus, minus,
        ...
        if group:
            ...
            if index is None:
                ...
                end_index = min(gt.shape[1], index + interval)
                if yp is None:
                    yp = torch.unsqueeze(self.yp[start_index: end_index].cuda(self.device), dim=0)
                    # (1 x 20 x 100k)
                # return best acc coordinate
                best_index_coord, acc = self.cal_acc_group(gt, yp, plus, minus)
                row, best_index = best_index_coord[0], best_index_coord[1]
    ...
    def eval(self, data, labels, w1, w2, b1, b2, batch_size):
        """
        ...
        if batch_size is None:
            p1 = (torch.sign(data.matmul(w1) + b1) + 1) / 2
...
if __name__ == '__main__':
    ...
    scd = SCD(nrows=0.2, nfeatures=1, w_inc1=0.17, w_inc2=0.1, tol=0.0000001, local_iter=10,
                 num_iters=100, interval=10, round=1, updated_features=20,
