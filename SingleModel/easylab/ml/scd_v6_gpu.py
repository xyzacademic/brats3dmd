...
class SCD(object):
    ...
    def __init__(self, nrows, nfeatures, w_inc=100, tol=0.001, local_iter=10,
                 ...
                 seed=2018, adjust_inc=False, n_jobs=4, num_gpus=0,
                 warm_start=False):
        """
        ...
        # self.step = torch.Tensor([1, 10, 1e2, 1e3, -1, -10, -1e2, -1e3, ])
        self.adjust_inc = adjust_inc
        self.inc_scale = w_inc
        # self.step = torch.Tensor([1,  1e2, 1e4, 1e6,
                                  # -1,  -1e2, -1e4, -1e6])
    ...
    def single_run(self, data, labels, plus, minus, val_data, val_labels,
        ...
        for i in range(self.num_iters):
            ...
            temp_w, temp_b = self.single_iteration(
                w, data[row_index].cuda(self.device),
                labels[row_index].cuda(self.device),
                plus, minus)

            temp_acc = self.eval(val_data.cuda(self.device),
    ...
    def get_best_w_and_b(self, data, labels, w, plus, minus, best_objective):
        ...
        while localit < self.local_iter and \
            ...
            #         w_inc = w_inc_group[j]
            #
            #         w_ = torch.repeat_interleave(w.reshape((-1, 1)),
            #                        self.updated_features, dim=1)
    ...
    def cal_acc_group(self, labels, yp, plus, minus):
        ...
        # best_index[0] = best_index[0].astype(np.int64)
        # best_index[1] = best_index[1].astype(np.int64)
        # ind = acc.argmax()
        # row = ind % acc.shape[1]
        # column = ind % acc.shape[0]
        return best_index, acc[best_index]
...
if __name__ == '__main__':
    ...
    scd = SCD(nrows=0.2, nfeatures=1, w_inc=1, tol=0.0001, local_iter=100,
              num_iters=100, round=2, interval=10, seed=2018, adjust_inc=False,
              updated_features=10, n_jobs=2)
    a = time()
