...
class SCD(object):
    ...
    def __init__(self, nrows, nfeatures, w_inc=100, tol=0.001, local_iter=10,
        ...
        self.step = torch.Tensor([1, 2, 3, 4, -1, -2, -3, -4])
        self.w_inc = None
    ...
    def train(self, train_data, train_labels, val_data=None, val_labels=None):
        ...
        train_labels = torch.from_numpy(train_labels).char()
        if val_data is None:
            val_data = train_data
        ...
        self.ref_full_index = torch.repeat_interleave(
            torch.arange(self.updated_features).view((-1, 1)), rows_sum, dim=1)
    ...
    def get_best_b(self, labels, projection, index, interval, plus, minus,
        ...
        if group:
            ...
            if index is None:
                ...
                yp = torch.unsqueeze(self.yp[start_index: end_index], dim=0)
                # return best acc coordinate
                best_index_coord, acc = self.cal_acc_group(gt, yp, plus, minus)
                row, best_index = best_index_coord[0], best_index_coord[1]
...
if __name__ == '__main__':
    ...
    scd = SCD(nrows=0.75, nfeatures=1, w_inc=1, tol=0.0001, local_iter=100,
              num_iters=100, round=16, interval=10, seed=2018, adjust_inc=False,
