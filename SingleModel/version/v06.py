...
class MPN(nn.Module):
    ...
    def __init__(self, num_channels=3, base_filters=16, num_classes=2, norm_axis='all'):
        ...
        self.shortconnect2 = Conv_blocks(channels=base_filters * self.k//2, n_layers=2, kernel_size=3,
                                        activation=nn.ReLU(inplace=True), k=self.k)
    ...
    def forward(self, x):
        ...
        short_out = self.shortconnect2(short_out)
        ...
        # out = torch.cat([wt, short_out], dim=1) * wt_prob
