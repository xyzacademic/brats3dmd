...
class MPN(nn.Module):
    ...
    def __init__(self, num_channels=3, base_filters=16, num_classes=2, norm_axis='all'):
        ...
        self.et_conv = nn.Conv3d(in_channels=base_filters*(1+self.k//2), out_channels=2, kernel_size=1, stride=1, padding=0,
                                 bias=True)
        self.ed_conv = nn.Conv3d(in_channels=base_filters*(1+self.k//2), out_channels=2, kernel_size=1, stride=1, padding=0,
                                 bias=True)
        self.ncr_conv = nn.Conv3d(in_channels=base_filters*(1+self.k//2), out_channels=2, kernel_size=1, stride=1, padding=0,
                                 bias=True)
        self.wt_conv = nn.Conv3d(in_channels=base_filters * (3+self.k//2), out_channels=2, kernel_size=1, stride=1, padding=0,
                                 bias=True)
        self.tc_conv = nn.Conv3d(in_channels=base_filters * (2+self.k//2), out_channels=2, kernel_size=1, stride=1, padding=0,
                                 bias=True)
