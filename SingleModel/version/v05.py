...
class Encoder(nn.Module):
    ...
    def __init__(self, num_channels=3, base_filters=16):
        ...
        #                         pool=nn.AvgPool3d(2, 2), k=self.k)
        self.conv5 = Conv_blocks(channels=base_filters * 8 * self.k, n_layers=2, kernel_size=3, activation=self.act,
                                 k=self.k)
    ...
    def forward(self, x):
        ...
        return (conv1, conv2, conv3, conv5)
...
class Decoder(nn.Module):
    ...
    def __init__(self, num_classes=2, base_filters=16):
        ...
        self.deconv1 = Upsample(in_channels=base_filters * 8, out_channels=base_filters * 4, kernel_size=2, stride=2,
                                bias=False)
        self.conv1 = Conv_blocks(channels=base_filters * 4, n_layers=2, kernel_size=3, activation=self.act, k=1)

        self.deconv2 = Upsample(in_channels=base_filters * 4, out_channels=base_filters * 2, kernel_size=2, stride=2,
                                bias=False)
        self.conv2 = Conv_blocks(channels=base_filters * 2, n_layers=2, kernel_size=3, activation=self.act, k=1)

        self.deconv3 = Upsample(in_channels=base_filters * 2, out_channels=base_filters, kernel_size=2, stride=2,
                                bias=False)
        self.conv3 = Conv_blocks(channels=base_filters, n_layers=2, kernel_size=3, activation=self.act, k=1)
    ...
    def forward(self, features):
        ...
        f3, f2, f1, f0 = features  # inverse variables' name order
