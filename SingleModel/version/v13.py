...
class MPN(nn.Module):
    ...
    def __init__(self, num_channels=3, base_filters=16, num_classes=2, norm_axis='all'):
        ...
        self.conv0 = nn.Conv3d(self.k*2, base_filters * self.k, 3, padding=1, bias=False, groups=self.k*2)
        self.bn = nn.InstanceNorm3d(num_features=base_filters * self.k, eps=1e-5, momentum=0.1, affine=AFFINE)
        self.shortconnect = Conv_blocks(channels=base_filters * self.k, n_layers=2, kernel_size=3,
                                        activation=nn.ReLU(inplace=True), k=self.k*2)
        ...
        self.et_conv = nn.Conv3d(in_channels=base_filters*(1+self.k), out_channels=2, kernel_size=1, stride=1, padding=0,
                                 bias=True)
        self.ed_conv = nn.Conv3d(in_channels=base_filters*(1+self.k), out_channels=2, kernel_size=1, stride=1, padding=0,
                                 bias=True)
        self.ncr_conv = nn.Conv3d(in_channels=base_filters*(1+self.k), out_channels=2, kernel_size=1, stride=1, padding=0,
                                 bias=True)
        self.wt_conv = nn.Conv3d(in_channels=base_filters * (3+self.k), out_channels=2, kernel_size=1, stride=1, padding=0,
                                 bias=True)
        self.tc_conv = nn.Conv3d(in_channels=base_filters * (2+self.k), out_channels=2, kernel_size=1, stride=1, padding=0,
                                 bias=True)
        self.final_conv = nn.Conv3d(in_channels=base_filters * (5+3+self.k), out_channels=num_classes, kernel_size=1, stride=1,
                                    padding=0,
                                    bias=True)
        self.apply(_weights_init)
    ...
    def forward(self, x):
        ...
        et_out = self.et_conv(torch.cat([et, short_out], dim=1))
        et_prob = F.softmax(et_out, dim=1)[:, 1:2]
        ed_out = self.ed_conv(torch.cat([ed, short_out], dim=1))
        ed_prob = F.softmax(ed_out, dim=1)[:, 1:2]
        ncr_out = self.ncr_conv(torch.cat([ncr, short_out], dim=1))
        ncr_prob = F.softmax(ncr_out, dim=1)[:, 1:2]
        wt = torch.cat([et, ed, ncr], dim=1)
        wt_out = self.wt_conv(torch.cat([wt, short_out], dim=1))
        wt_prob = F.softmax(wt_out, dim=1)[:, 1:2]
        tc = torch.cat([et, ncr], dim=1)
        tc_out = self.tc_conv(torch.cat([tc, short_out], dim=1))
        tc_prob = F.softmax(tc_out, dim=1)[:, 1:2]

        # short_out = F.relu(short_out, inplace=True)
        out = torch.cat([et*et_prob, ed*ed_prob, ncr*ncr_prob, wt*wt_prob, tc*tc_prob,
                         short_out*wt_prob], dim=1)
...
if __name__ == '__main__':
    import os
    x = torch.randn((1, 8, 160, 192, 144)).cuda()
    net = MPN(num_channels=4, base_filters=4, num_classes=5, norm_axis='all')
    net.cuda()
    y = net(x)
